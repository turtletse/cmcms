...
BEGIN
    ...
    IF (in_user_id<>'SYSADM') THEN
		...
		SELECT last_update_dtm INTO cmcms_update_dtm FROM user_account WHERE user_id = in_user_id;
		IF (cmcis_update_dtm IS NOT NULL AND cmcms_update_dtm IS NOT NULL) THEN
			...
			ELSEIF(cmcms_update_dtm > cmcis_update_dtm) THEN
				SELECT hashed_password, chin_name, eng_name INTO new_pw, new_chi_name, new_eng_name FROM user_account WHERE user_id = in_user_id;
				CALL cmcis.common_user_update(in_user_id, new_pw, new_chi_name, new_eng_name);
				SELECT last_update_dtm INTO cmcis_update_dtm FROM cmcis.common_user WHERE user_id = in_user_id;
				UPDATE user_account
