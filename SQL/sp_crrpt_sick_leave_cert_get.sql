DROP procedure if EXISTS sp_crrpt_sick_leave_cert_get;
delimiter $$
CREATE PROCEDURE sp_crrpt_sick_leave_cert_get (
	IN in_cert_id INT
)
BEGIN
	SELECT cert_id,
    ...
    FROM sick_leave_cert JOIN consultation_record ON consultation_id = cons_id 
