DROP procedure if EXISTS sp_medical_history_get;
delimiter $$
CREATE PROCEDURE sp_medical_history_get (IN in_clinic_id VARCHAR(10), IN in_pat_id INT)
BEGIN
    ...
    WHERE clinic_id = in_clinic_id AND isFinished > 0 AND consultation_record.patient_id = in_pat_id;
...
-- CALL sp_medical_history_get('CITYC', 1)