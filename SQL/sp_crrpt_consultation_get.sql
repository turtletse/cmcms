...
BEGIN
	...
	select clinic_id, cons_id, CONCAT('*',cons_id,'*') cons_id_barcode, CONCAT(patient_record.chin_name, ' (', patient_record.patient_id,')') pat_name, user_account.chin_name dr_name, reg_no, dx_desc, dr_rmk, DATE_FORMAT(last_update_dtm, '%d/%m/%Y %T') last_update_dtm, pres_id pres_id_data_str
...
-- CALL sp_crrpt_consultation_get(2)