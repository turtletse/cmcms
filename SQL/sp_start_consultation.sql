...
BEGIN
    IF(SELECT COUNT(*) FROM consultation_record WHERE clinic_id = in_clinic_id AND dr_id = in_dr_id AND patient_id = in_pat_id AND isFinished = 0)=0 THEN
    ...
    WHERE clinic_id = in_clinic_id AND dr_id = in_dr_id AND patient_id = in_pat_id AND isFinished = 0;
END $$
