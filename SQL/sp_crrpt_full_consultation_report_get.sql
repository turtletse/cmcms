...
BEGIN
	...
	SELECT cons_id, clinic_id, dr_id, user_account.chin_name dr_name, consultation_record.patient_id, CASE WHEN patient_record.chin_name IS NULL OR LENGTH(TRIM(patient_record.chin_name)) = 0 THEN patient_record.eng_name ELSE patient_record.chin_name END pat_name, DATE_FORMAT(first_record_dtm, '%d/%m/%Y %T') first_record_dtm, DATE_FORMAT(consultation_record.last_update_dtm, '%d/%m/%Y %T') last_update_dtm, ex_desc, diff_desc, dx_desc, prescription.pres_id, method_of_treatment, drug_name, CONCAT_WS(' ', TRIM(TRAILING '.' FROM TRIM(TRAILING '0' FROM dosage)), dosage_unit.unit_desc) dosage, CASE WHEN method_desc = '-' THEN '正常煎煮' ELSE method_desc end method_desc, display_order, no_of_dose, acupuncture_desc, dr_rmk
