DROP PROCEDURE IF EXISTS sp_patient_queue_staff_call_answered;

DELIMITER $$
CREATE PROCEDURE sp_patient_queue_staff_call_answered (
...
BEGIN
	...
	START TRANSACTION;
		...
		IF @tablock = in_user_id THEN
			...
			PREPARE stmt FROM @sql_str;
			EXECUTE stmt;
			SET @sql_str = CONCAT('UPDATE ',tabname,' SET patient_status = 20, doctor_in_charge =''', in_moic_id, ''' WHERE patient_status = 10 AND enter_dtm = @dtm;');
			PREPARE stmt FROM @sql_str;
			EXECUTE stmt;
