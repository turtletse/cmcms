...
BEGIN
	SELECT clinic.clinic_id, 
        ...
        CASE WHEN in_clinic_id = 'ALL' OR clinic.clinic_id = in_clinic_id THEN CAST(IFNULL(COUNT(DISTINCT user_id), 0) AS CHAR) ELSE '***' END user_cnt
    FROM clinic
		LEFT JOIN user_clinic_role_mapping ON clinic.clinic_id = user_clinic_role_mapping.clinic_id
